  ...
  constructor(props){
    ...
    this.state={
      ...
      inativate = async v => {
        ...
        await api.put("/api/video/inactive/"+v._id);    
       let videos= this.state.videos.filter(ev=>ev._id!==v._id);
            ...
            <Card>
              ...
              <Loader show={this.state.processing} message={spinner}  backgroundStyle={{color:'white'}} messageStyle={{margin:'auto',padding:'10px'}}>
                <Table responsive hover size="sm" >
                 ...
                 <tr key={index}>  
                 <th scope="row">        
              <img src={`${baseURL}/${video.screenshot}`} style={{height:'60px',width:'120px', "background-size": 'cover'}}  /></th>    
              ...
              <td>{video.creationDate.substring(0, 10)}</td> 
              <td>{video.views}</td>
              <td>{video.comments.length}</td>
              <td> <button type="button" class="btn btn-outline-danger btn-sm" onClick={this.toggleFade.bind(this,video)} ><i className="fa fa-close"></i></button></td>
